...
public class HttpRequestTest {
    ...
    public void コンストラクタの引数にURIを指定して呼び出すと_HttpRequestが生成されること() {
        ...
        // Setup
        URI uri = URIUtil.toURI("http://www.ambrosoli.jp/test-server/"); //$NON-NLS-1$
    ...
    public void コンストラクタの引数にnullを渡すと_IllegalArgumentExceptionが発生すること() {
        ...
        this.expectedException.expectMessage(is(equalTo("URI may not to be null."))); //$NON-NLS-1$
    ...
    public void addHeadersの引数にNameValueObject配列を渡すと_HttpRequestにNameValueObjectのListが保持されること() {
        ...
        values.add(new NameValueObject("name4", "value4")); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void addHeadersを２回呼び出すと_上書きではなく追加で保持されること() {
        ...
        values1.add(new NameValueObject("name3", "value3")); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        values2.add(new NameValueObject("name6", "value6")); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void addHeadersにnullを渡すと_HttpRequestに追加されないこと() {
        ...
        // Exercise
        sut.addHeaders(Arrays.asList(new HttpHeader("name", "value"))); //$NON-NLS-1$ //$NON-NLS-2$
        sut.addHeaders(null);
    ...
    public void addHeaderの引数にNameValueObjectを渡すと_HttpRequestに追加されること() {
        ...
        // Setup
        HttpRequest sut = new HttpRequest(URIUtil.toURI("http://www.ambrosoli.jp/")); //$NON-NLS-1$
        NameValueObject header = new NameValueObject("name", "value"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void addHeaderを複数回呼び出すと_上書きではなく追加で保持されること() {
        ...
        // Exercise
        sut.addHeader(new HttpHeader("name1", "value1")); //$NON-NLS-1$ //$NON-NLS-2$
        sut.addHeader(new HttpHeader("name2", "value2")); //$NON-NLS-1$ //$NON-NLS-2$
        sut.addHeader(new HttpHeader("name3", "value3")); //$NON-NLS-1$ //$NON-NLS-2$
        sut.addHeader(new HttpHeader("name4", "value4")); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void setAuthInfoの引数にAuthInfoを指定した場合_HttpRequestにAuthInfoが保持されること() {
        ...
        // Exercise
        AuthInfo authInfo = new AuthInfo(AuthType.BASIC, "user.name", "user.password"); //$NON-NLS-1$ //$NON-NLS-2$
